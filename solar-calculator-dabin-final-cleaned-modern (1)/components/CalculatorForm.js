  ...
  return (
    <div className="grid grid-cols-1 gap-3">
      {[
        ...
        <div key={name}>
          ...
          <input
            ...
            className="w-full border rounded-lg p-2 bg-white/80 focus:outline-none focus:ring-2 focus:ring-blue-400"
          />
        </div>
      ))}
      ...
      <div className="mt-2 text-gray-700 text-sm space-y-1">
        ...
        <div>회수기간 (투자금 기준): {typeof payback === 'number' ? `${payback} 년` : '-'} </div>
      </div>
    </div>
  );
}